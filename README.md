...
In your favorite editor (e.g., `vi`, `nano`), examine the cluster creation script `create_cluster.sh`. You may choose to alter some of the parameters that are described in the following table depending on your objectives.

| Variable / Option | Default Value | Description                                    |
|----------------- |------------- |---------------------------------------------- |
| FLAVOR            | m3.small      | VM size for worker nodes (CPU/RAM)             |
| N\_NODES          | 1             | Number of worker nodes                         |
| min\_node\_count  | 1             | Minimum node count when autoscaling is enabled |
| max\_node\_count  | 5             | Maximum node count when autoscaling is enabled |
